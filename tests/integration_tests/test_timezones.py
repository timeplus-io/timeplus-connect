...
def test_basic_timezones(test_client: Client):
    row = test_client.query("SELECT to_datetime('2022-10-25 10:55:22', 'America/Chicago') as chicago," +
                            "to_datetime('2023-07-05 15:10:40') as utc",
    ...
    if test_client.min_version('20'):
        row = test_client.query("SELECT to_datetime64('2022-10-25 10:55:22.789123', 6, 'America/Chicago')",
                                query_tz='America/Chicago').first_row
        assert row[0].tzinfo == chicago_tz
        assert row[0].hour == 10
        assert row[0].day == 25
...
def test_server_timezone(test_client: Client):
    ...
    try:
        date = test_client.query('SELECT to_datetime(%s) as st', parameters=[test_datetime]).first_row[0]
...
def test_column_timezones(test_client: Client):
    date_tz64 = "to_datetime64('2023-01-02 15:44:22.7832', 6, 'Asia/Shanghai')"
    if not test_client.min_version('20'):
        date_tz64 = "to_datetime('2023-01-02 15:44:22', 'Asia/Shanghai')"
    ...
    if test_client.min_version('20'):
        row = test_client.query("SELECT to_datetime('2022-10-25 10:55:22', 'America/Chicago') as chicago," +
                                "to_datetime64('2023-01-02 15:44:22.7832', 6, 'Asia/Shanghai') as china").first_row
...
def test_local_timezones(test_client: Client):
    ...
    test_client.apply_server_timezone = False
    try:
        row = test_client.query("SELECT to_datetime('2022-10-25 10:55:22'," +
                                "'America/Chicago') as chicago," +
                                "to_datetime('2023-07-05 15:10:40') as raw_utc_dst," +
                                "to_datetime('2023-07-05 12:44:22', 'UTC') as forced_utc," +
                                "to_datetime('2023-12-31 17:00:55') as raw_utc_std").first_row
        # if test_client.protocol_version:
...
def test_naive_timezones(test_client: Client):
    row = test_client.query("SELECT to_datetime('2022-10-25 10:55:22', 'America/Chicago') as chicago," +
                            "to_datetime('2023-07-05 15:10:40') as utc").first_row
...
def test_timezone_binding_client(test_client: Client):
    ...
    try:
        server_time = test_client.query(
            'SELECT to_datetime(%(dt)s) as dt', parameters={'dt': denver_time}).first_row[0]
    ...
    server_time = test_client.query(
        'SELECT to_datetime(%(dt)s) as dt', parameters={'dt': naive_time}).first_row[0]
    assert server_time.astimezone(pytz.UTC) == naive_time.astimezone(pytz.UTC)

    utc_time = datetime(2023, 3, 18, 16, 4, 25, tzinfo=pytz.UTC)
    server_time = test_client.query(
        'SELECT to_datetime(%(dt)s) as dt', parameters={'dt': utc_time}).first_row[0]
...
def test_timezone_binding_server(test_client: Client):
    ...
    try:
        server_time = test_client.query(
            'SELECT to_datetime({dt:DateTime}) as dt', parameters={'dt': denver_time}).first_row[0]
        assert server_time == denver_time
    finally:
        os.environ['TZ'] = 'UTC'
        time.tzset()
    ...
    server_time = test_client.query(
        'SELECT to_datetime({dt:DateTime}) as dt', parameters={'dt': naive_time}).first_row[0]
    assert naive_time.astimezone(pytz.UTC) == server_time.astimezone(pytz.UTC)

    utc_time = datetime(2020, 3, 18, 16, 4, 25, tzinfo=pytz.UTC)
    server_time = test_client.query(
        'SELECT to_datetime({dt:DateTime}) as dt', parameters={'dt': utc_time}).first_row[0]
    assert server_time.astimezone(pytz.UTC) == utc_time
